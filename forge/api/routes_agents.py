...
from forge.api.schemas import (ManagedAgentGrantRequest, ManagedAgentRunRequest,
                               ManagedAgentSpecRequest,
                               ManagedAgentTemplateRequest,
                               ManagedAgentUpdateRequest,
                               ManagedAgentVersionRequest)


@router.get("/agents/managed/templates")
async def managed_templates(current: Authed = Depends(authed_mutation),
                            plane: ControlPlane = Depends(get_plane)):
    return plane.managed_templates(current.session)


@router.get("/agents/managed/runs/approvals")
async def managed_run_approvals(current: Authed = Depends(authed_mutation),
                                plane: ControlPlane = Depends(get_plane)):
    return {"approvals": plane.list_managed_approvals(current.session)}


@router.post("/agents/managed/runs/approvals/{approval_id}/approve",
             dependencies=[rate_limit("agents")])
async def managed_approve_run(approval_id: str,
                              current: Authed = Depends(authed_mutation),
                              plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.decide_managed_approval(current.session, approval_id,
                                             True)
...
@router.post("/agents/managed/runs/approvals/{approval_id}/deny",
             dependencies=[rate_limit("agents")])
async def managed_deny_run(approval_id: str,
                           current: Authed = Depends(authed_mutation),
                           plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.decide_managed_approval(current.session, approval_id,
...
async def managed_list(current: Authed = Depends(authed_mutation),
                       plane: ControlPlane = Depends(get_plane)):
    return plane.managed_list(current.session)


@router.post("/agents/managed", dependencies=[rate_limit("agents")])
async def managed_create(body: ManagedAgentSpecRequest,
                         current: Authed = Depends(authed_mutation),
                         plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_create(current.session, body.spec,
                                    template=body.template)
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.post("/agents/managed/from-template",
             dependencies=[rate_limit("agents")])
async def managed_create_template(body: ManagedAgentTemplateRequest,
                                  current: Authed = Depends(authed_mutation),
                                  plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_create_from_template(
            current.session, body.template, body.name, body.purpose)
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.get("/agents/managed/{name}")
async def managed_get(name: str,
                      current: Authed = Depends(authed_mutation),
                      plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_get(current.session, name)
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.patch("/agents/managed/{name}", dependencies=[rate_limit("agents")])
async def managed_update(name: str, body: ManagedAgentUpdateRequest,
                         current: Authed = Depends(authed_mutation),
                         plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_update(current.session, name, body.spec,
                                    bump=body.bump, reason=body.reason)
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.post("/agents/managed/{name}/version",
             dependencies=[rate_limit("agents")])
async def managed_version(name: str, body: ManagedAgentVersionRequest,
                          current: Authed = Depends(authed_mutation),
                          plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_version(current.session, name,
                                     version=body.version, bump=body.bump,
                                     reason=body.reason)
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.post("/agents/managed/{name}/validate",
             dependencies=[rate_limit("agents")])
async def managed_validate(name: str,
                           current: Authed = Depends(authed_mutation),
                           plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_validate(current.session, name)
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.post("/agents/managed/{name}/test",
             dependencies=[rate_limit("agents")])
async def managed_test(name: str,
                       current: Authed = Depends(authed_mutation),
                       plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_test(current.session, name)
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.post("/agents/managed/{name}/enable",
             dependencies=[rate_limit("agents")])
async def managed_enable(name: str,
                         current: Authed = Depends(authed_mutation),
                         plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_enable(current.session, name)
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.post("/agents/managed/{name}/pause",
             dependencies=[rate_limit("agents")])
async def managed_pause(name: str,
                        current: Authed = Depends(authed_mutation),
                        plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_pause(current.session, name)
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.post("/agents/managed/{name}/disable",
             dependencies=[rate_limit("agents")])
async def managed_disable(name: str,
                          current: Authed = Depends(authed_mutation),
                          plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_disable(current.session, name)
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.post("/agents/managed/{name}/retire",
             dependencies=[rate_limit("agents")])
async def managed_retire(name: str,
                         current: Authed = Depends(authed_mutation),
                         plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_retire(current.session, name)
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.post("/agents/managed/{name}/grant",
             dependencies=[rate_limit("agents")])
async def managed_grant(name: str, body: ManagedAgentGrantRequest,
                        current: Authed = Depends(authed_mutation),
                        plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_grant(current.session, name, {
            "resource": body.resource, "operation": body.operation,
            "scope": body.scope, "effect": body.effect})
    except InvalidRequest as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from None


@router.post("/agents/managed/{name}/run",
             dependencies=[rate_limit("agents")])
async def managed_run(name: str, body: ManagedAgentRunRequest,
                      current: Authed = Depends(authed_mutation),
                      plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.managed_run(
