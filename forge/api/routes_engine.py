...
from forge.api.schemas import (EngineCreateRequest, EngineGrantRequest,
                               ...
                               EngineUpdateRequest, EngineVersionRequest)
...
async def engine_create(body: EngineCreateRequest,
    ...
    try:
        return plane.engine_create(
            ...
            spec=body.spec, overrides=body.overrides)
...
@router.patch("/engine/agents/{name}",
              dependencies=[rate_limit("agents")])
async def engine_update(name: str, body: EngineUpdateRequest,
                        current: Authed = Depends(authed_mutation),
                        plane: ControlPlane = Depends(get_plane)):
    try:
        return plane.engine_update(current.session, name, body.spec,
                                   reason=body.reason)
    except InvalidRequest as exc:
        raise _bad(exc) from None
