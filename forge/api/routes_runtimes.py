...
def _monitor(plane: ControlPlane):
    ...
    if monitor is None:
        raise HTTPException(status_code=503,
                            detail="Runtime monitor is not configured.")
    return monitor
...
async def runtime_status(
    ...
    """Return sanitized runtime state; credentials are never exposed."""
    del current
    return _monitor(plane).snapshot()
...
async def runtime_check(
    ...
    """Force one exact-model verification pass."""
    del current
    return _monitor(plane).tick(force=True)
