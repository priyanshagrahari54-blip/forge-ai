...
class _NoRedirect(urllib.request.HTTPRedirectHandler):
    """Refuse redirects: signed auth headers must never follow them.

    ``urllib`` follows 3xx by default and would re-send the
    ``X-Forge-*`` signature headers to the redirect target — a header-
    leak bug on any hostile/misconfigured redirect. Redirects are
    reported as transport failures; configure the real server URL
    instead.
    """
...
#: Installed once per process (idempotent).
...
def urllib_sender(method: str, url: str, headers: dict, body: bytes,
    ...
    Hardened: redirects are refused (signature headers would leak to
    the redirect target), responses are strictly bounded, and every
    failure is normalized to :class:`TransportError`.
    """
...
class LinkTransport:
    ...
    Thread-safe: the UI poller and the heartbeat thread share this
    object, so session state is mutated only under ``self._lock`` and
    at most one handshake runs at a time.
    """
    ...
    def connect(self) -> dict[str, Any]:
        ...
        this process. Concurrent callers collapse into one handshake
        (the second waits, then observes a live session).
        """
        with self._lock:
            if self.connected:
                return {}
    ...
    def _signed_raw(self, method: str, path: str, url: str,
        ...
        signature = protocol.sign_request(
            key, method, path, payload, timestamp, nonce)
