...
from forge.client.estimator import TaskEstimate
...
class ForgeClient:
    ...
    def _resources_snapshot(self, requirement: str) -> ResourceSnapshot:
        est = TaskEstimate(kind="inspect", chars=len(requirement),
                           words=0, needs_model=False, signals=())
        del est  # resources don't depend on the task; explicit param only
        return self._probe()
    ...
    def snapshot(self, *, selected_task: str = "", full: bool = False) \
        ...
        if selected_task and not selected_task.startswith("local-"):
            ...
            except (LinkError, AuthError) as exc:
