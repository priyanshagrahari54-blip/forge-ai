...
class LinkStore:
    ...
    def __init__(self, db: Database) -> None:
        ...
        with self._lock:
            ...
            """)
            db.execute("""
                CREATE TABLE IF NOT EXISTS link_handshake_failures (
                    client_id  TEXT NOT NULL,
                    at         REAL NOT NULL
                )
            """)
            db.execute(
                "CREATE INDEX IF NOT EXISTS idx_link_hs_fail_client"
                " ON link_handshake_failures (client_id, at)")
    ...
    def nonce_seen(self, client_id: str, nonce: str, *,
        ...
        Race-safe: two concurrent requests carrying the same nonce both
        count as "seen" from the first INSERT on (the loser of the race
        hits the primary key and is rejected, never crashes).
        """
        import sqlite3
        with self._lock:
            ...
            try:
                ...
                # Lost the race with a concurrent identical nonce.
                return True
            self._db.execute(
    ...
    # -- handshake abuse control (rate limit + failure lockout) ----------------

    #: Failed handshakes tolerated before the client id is locked out.
    MAX_HANDSHAKE_FAILURES = 5
    #: Lockout duration after too many failures (seconds).
    HANDSHAKE_LOCKOUT_SECONDS = 300.0

    def record_handshake_failure(self, client_id: str, *,
                                 now: float) -> int:
        """Record one failed handshake; returns the failure count.

        A success clears the counter (see :meth:`clear_handshake_failures`).
        Bounded: only the trailing ``MAX_HANDSHAKE_FAILURES`` entries are
        retained per client.
        """
        with self._lock:
            self._db.execute(
                "INSERT INTO link_handshake_failures (client_id, at)"
                " VALUES (?, ?)", (client_id, now))
            self._db.execute(
                "DELETE FROM link_handshake_failures WHERE client_id = ?"
                " AND rowid NOT IN (SELECT rowid FROM"
                " link_handshake_failures WHERE client_id = ?"
                " ORDER BY rowid DESC LIMIT ?)",
                (client_id, client_id, self.MAX_HANDSHAKE_FAILURES))
            row = self._db.query_one(
                "SELECT COUNT(*) AS n FROM link_handshake_failures"
                " WHERE client_id = ? AND at > ?",
                (client_id, now - self.HANDSHAKE_LOCKOUT_SECONDS))
            return int(row["n"]) if row is not None else 0

    def clear_handshake_failures(self, client_id: str) -> None:
        with self._lock:
            self._db.execute(
                "DELETE FROM link_handshake_failures WHERE client_id = ?",
                (client_id,))

    def handshake_locked(self, client_id: str, *, now: float) -> bool:
        """True while the client id is locked out for repeated failures.

        Revoked or unknown clients are never reported locked (they fail
        authentication regardless); lockout only throttles known ids.
        """
        with self._lock:
            row = self._db.query_one(
                "SELECT COUNT(*) AS n FROM link_handshake_failures"
                " WHERE client_id = ? AND at > ?",
                (client_id, now - self.HANDSHAKE_LOCKOUT_SECONDS))
        return row is not None and int(row["n"]) >= self.MAX_HANDSHAKE_FAILURES
