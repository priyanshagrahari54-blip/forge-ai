...
class LinkService:
    ...
    Reuses the ControlPlane's shared database connection when available
    (one writer, no cross-connection lock contention); falls back to a
    dedicated connection only for planes that expose no shared handle.
    """
    ...
    def __init__(self, plane: ControlPlane,
        ...
        shared_db = getattr(plane, "_db", None)
        if isinstance(shared_db, Database):
            # Shared connection with the control plane (WAL, lock-guarded).
            self._owns_db = False
            self.store = LinkStore(shared_db)
        else:
            self._owns_db = True
            self.store = LinkStore(Database(plane.config.db_path))

    def close(self) -> None:
        """Release resources owned by this service (shared DB untouched)."""
        if self._owns_db:
            try:
                self.store._db.close()  # noqa: SLF001 - own connection
            except Exception:
                pass

    # -- audit (same JSONL trail the cockpit uses) ------------------------------

    def _audit(self, actor: str, operation: str, allowed: bool, *,
               task_id: str = "", reason: str = "") -> None:
        """Best-effort audit record for a link action (never raises)."""
        try:
            self.plane._audit(  # noqa: SLF001 - host-plane audit helper
                actor, "link:" + ("client" if not task_id else "task"),
                operation, allowed, task_id=task_id, reason=reason)
        except Exception:
            pass
    ...
    def register_client(self, client_id: str, project_id: str, *,
        ...
        The project must exist — registration never creates projects.
        """
        protocol.require_client_id(client_id)
        if project_id not in self.plane.projects:
            raise RequestError(
                f"unknown project: {project_id!r} (mount it first)",
                code="PROJECT_NOT_FOUND")
        if max_mode not in _VALID_MODES:
            raise RequestError(f"invalid max_mode: {max_mode!r}",
        ...
        self._audit("operator", "register_client", True,
                    reason=f"client {client_id} project {project_id} "
                           f"max_mode {max_mode}")
    ...
    def challenge(self, client_id: str, nonce_client: str, *,
        ...
        *same* reply shape with an unusable random nonce + random salt,
        ...
        if not _bounded(client_id, 64) or not _bounded(nonce_client, 64):
    ...
    def handshake(self, client_id: str, nonce_client: str, proof: str, *,
        ...
        authentication failures are :class:`AuthError` (constant
        message) and count toward the per-client failure lockout; a
        locked-out client keeps receiving :class:`AuthError` until the
        lockout expires (defeats online proof-guessing loops).
        """
        now = time.time() if now is None else now
        if not _bounded(client_id, 64) or not _bounded(proof, 128):
            raise AuthError("authentication failed")
        client = self.store.get_client(client_id) \
            if protocol.valid_client_id(client_id) else None
        if client is None or client.get("status") != "active":
            raise AuthError("authentication failed")
        if self.store.handshake_locked(client_id, now=now):
            self._audit(f"link-{client_id}", "handshake", False,
                        reason="locked out (repeated failures)")
            raise AuthError("authentication failed")
        nonce_client = _clean_nonce(nonce_client)
        if nonce_client is None or not isinstance(proof, str) or not proof:
            self._record_handshake_outcome(client_id, ok=False, now=now)
            raise AuthError("authentication failed")
        ...
        try:
            ...
            # Server-side capacity/policy refusal — an honest 4xx, not
            # an authentication failure (never counts toward lockout).
            raise RequestError(str(exc) or exc.code,
        ...
        self.store.clear_handshake_failures(client_id)
        self._record_handshake_outcome(client_id, ok=True, now=now)
        self._audit(f"link-{client_id}", "handshake", True,
    ...
    def _record_handshake_outcome(self, client_id: str, *, ok: bool,
                                  now: float) -> None:
        if ok:
            self.store.clear_handshake_failures(client_id)
            return
        failures = self.store.record_handshake_failure(client_id, now=now)
        self._audit(f"link-{client_id}", "handshake", False,
    ...
    def submit_task(self, session: Session, client_id: str, requirement: str,
        ...
        try:
            ...
            # Validation (400), conflict (409), policy refusal: honest
            # codes instead of an opaque 500.
            raise RequestError(str(exc) or exc.code,
                               code=getattr(exc, "code", "INVALID_REQUEST")) \
                from None
        self.store.set_task_origin(run.id, client_id, execution, decision)
        self._audit(f"link-{client_id}", "submit_task", True,
                    task_id=run.id,
                    reason=f"execution={execution} "
    ...
    def state_snapshot(self, session: Session, *, since_seq: int = 0,
        ...
        if focus:
            try:
                # Bounded fetch; the cursor is the LAST RETURNED seq, so
                # a truncated batch is re-fetched next poll instead of
                # being skipped.
                events, store_latest = self.get_task_events(
                    session, focus, after=since_seq)
                latest = events[-1]["seq"] if events else store_latest
            except RequestError:
...
def _bounded(value: Any, limit: int) -> bool:
    """Input bound: must be a str (when not None) and short enough."""
    if value is None:
        return True
