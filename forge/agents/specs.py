...
* tools (allowlist — the agent may touch nothing else)
* permissions (requested grants — granted only by an operator/policy,
...
OPERATION_RE = re.compile(r"^[a-z_]{1,32}$")
...
#: Tools a created agent may request. Anything else is refused at
#: validation time, and the mediated runtime enforces the allowlist again.
...
#: Permission resources a created agent may request. Mirrors the A33
#: resource vocabulary (kept as plain strings so specs stay portable).
...
class PermissionRequestSpec:
    ...
    def validate(self) -> list[str]:
        issues: list[str] = []
        if self.resource not in KNOWN_RESOURCES:
            issues.append(
                "permission resource %r is unknown; expected one of %s"
                % (self.resource, sorted(KNOWN_RESOURCES)))
        if not isinstance(self.operation, str) \
                or not OPERATION_RE.match(self.operation):
            issues.append(
                "permission operation %r must match [a-z_]{1,32}"
                % (self.operation,))
        if not isinstance(self.scope, str) or not self.scope.strip() \
                or len(self.scope) > MAX_SCOPE:
            issues.append("permission scope must be 1-%d characters"
                          % MAX_SCOPE)
...
class AgentSpec:
    ...
    purpose: str
    capabilities: tuple[str, ...] = ("coding",)
    tools: tuple[str, ...] = ("filesystem_read", "memory")
...
TEMPLATES: dict[str, dict[str, Any]] = {
    "coding": {
        ...
        "tools": ["filesystem_read", "filesystem_write", "terminal",
        ...
        "permissions": [
            ...
            _perm("git", "stage", "src/**",
                  "MEDIUM", "Stage explicitly validated files only."),
        ],
    ...
    "research": {
        ...
        "tools": ["filesystem_read", "browser_search", "network_fetch",
                  "memory"],
        "permissions": [
            _perm("filesystem", "read", "**",
                  "LOW", "Read-only repository investigation."),
            _perm("browser", "search", "public-web",
                  "LOW", "Look up public documentation and sources."),
            _perm("network", "fetch", "docs/**",
    ...
    "security": {
        ...
        "tools": ["filesystem_read", "terminal", "memory"],
        "permissions": [
            _perm("filesystem", "read", "**",
    ...
    "documentation": {
        ...
        "tools": ["filesystem_read", "filesystem_write", "memory"],
        "permissions": [
            _perm("filesystem", "read", "**",
