...
* Versions are explicit semver records with spec hashes and notes; history
  is append-only.
* Exports carry the spec and version only — never grants, never executors —
  so importing a package can never smuggle execution power.
...
#: Honest executor bindings: a package is runnable only when its role maps
#: to an executor that genuinely exists. Roles without an entry stay
#: unbound and can never be enabled.
ROLE_EXECUTORS = {
    "coding": "coder",
    "debugging": "debugger",
    "testing": "tester",
...
class AgentPackage:
    ...
    def manifest(self) -> dict[str, Any]:
        """Stable package summary: identity, version, lifecycle, bindings."""
    ...
    def from_dict(cls, payload: Any) -> "AgentPackage":
        ...
        if package.bound and not package.executor:
            raise ValueError("bound packages must name an executor")
        return package
...
class AgentCreationEngine:
    ...
    def create_from_spec(self, payload: dict[str, Any], *,
        ...
        return self._store(spec, created_by=created_by, bind=bind)

    def create_from_template(self, template: str, name: str, *,
                             created_by: str = "",
                             overrides: dict[str, Any] | None = None,
                             bind: bool = True) -> AgentPackage:
        spec = build_template(template, name, overrides)
        return self._store(spec, created_by=created_by, bind=bind)

    def _store(self, spec: AgentSpec, *, created_by: str,
               bind: bool) -> AgentPackage:
        ...
        executor = ROLE_EXECUTORS.get(spec.role, "") if bind else ""
        now = time.time()
        package = AgentPackage(
            name=spec.name, spec=spec.to_dict(), state="created",
            executor=executor, bound=bool(executor),
    ...
    def _transition(self, package: AgentPackage, to_state: str,
        ...
        package.transitions.append(
            {"from": package.transitions[-1]["to"] if package.transitions
             else "", "to": to_state, "at": package.updated_at,
    ...
    def enable(self, name: str, *, actor: str = "") -> AgentPackage:
        package = self.get(name)
        if not package.bound:
            raise ValueError(
                "agent %r has no bound executor; it is a specification, "
                "not a runnable agent" % (name,))
    ...
    # -- portable packages (grants and executors never travel) -------------
    ...
    def import_package(self, payload: Any, *,
        ...
        # Imports always arrive unbound and ungranted: execution power is
        # local and can never be smuggled across a package boundary.
