...
* **Checkpoints** — a checkpoint is captured before any mutation; failed
  verification rolls the changed files back.
...
class GatedAgentRuntime:
    ...
    MUTATING_TOOLS = frozenset({"filesystem_write", "terminal", "git"})

    def __init__(self, *, fabric: Any = None, policy_gate: Any = None,
                 tool_runtime: Any = None, memory_root: str = "",
                 project_root: str = ".", verification: Any = None,
    ...
    def execute_tool(self, package: Any, tool: str, run_id: str = "",
        ...
        if tool in self.MUTATING_TOOLS and self.policy_gate is not None:
            outcome = self.policy_gate.evaluate(
                "write_file", path=str(kwargs.get("path", "")),
                tool=tool, risk="MEDIUM", capability=tool,
                agent=agent_identity(name))
            decision = getattr(outcome, "decision", outcome)
            if str(decision) != "PolicyDecision.ALLOW" \
                    and getattr(decision, "value", decision) != "ALLOW":
                raise MediationError(
                    "GATE_DENIED",
                    "policy gate refused %s for agent %r: %s"
                    % (tool, name,
                       getattr(outcome, "reason", decision)))
        ...
        return {"tool": tool, "success": bool(result.success),
    ...
    def rollback(self, checkpoint: Any, changed: list[str]) -> bool:
        if checkpoint is None or self.checkpoint_manager is None:
            return False
        self.checkpoint_manager.rollback(checkpoint, list(changed))
        return True

    # -- internals ---------------------------------------------------------

    def _route_model(self, requirement: str, capability: str,
                     model_req: dict[str, Any]) -> dict[str, Any]:
        from forge.models.request import ModelRequest

        request = ModelRequest(
            prompt=requirement, capability=capability or "coding",
