...
and are labeled as such. Provider calls never masquerade as success:
every call returns a classified ``state`` of ``SUCCESS`` /
``PROVIDER_ERROR`` / ``TIMEOUT`` / ``RATE_LIMITED`` / ``AUTH_ERROR`` /
``POLICY_DENIED`` / ``UNAVAILABLE``.
"""
...
class WebSearchResult:
    """One search result from the web (untrusted external input)."""
...
class WebSearchProvider:
    ...
    def search(self, query: str) -> tuple[str, list[WebSearchResult]]:
        ...
        if self._searxng_url:
            ...
            return "UNAVAILABLE", []
        if state == "SUCCESS" and not results:
            return "PROVIDER_ERROR", []
    ...
    def _openai_post(self, url: str, body: dict[str, Any],
        ...
        """POST JSON to the OpenAI API through the SSRF-safe fetcher.

        OpenAI API access is done with a narrowly-scoped HTTPS policy.
        Returns ``(state, parsed_json_or_None)``.
        """
        # The SSRF module is GET-only by design; OpenAI calls are made with
        # urllib against the pinned, validated OpenAI host (operator
        # configuration, not user input), with strict timeouts and bounded
        # responses.
