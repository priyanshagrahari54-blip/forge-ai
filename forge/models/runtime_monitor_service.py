...
class RuntimeMonitorService:
    ...
    def start(self) -> None:
        with self._lock:
            ...
            self._stop.clear()
            self._thread = threading.Thread(target=self._run, name="forge-runtime-monitor", daemon=True)
            self._thread.start()

    def stop(self, *, wait: bool = True) -> None:
        with self._lock:
            thread = self._thread; self._thread = None; self._stop.set()
        ...
        with self._lock:
            ...
            for runtime in self.registry.items():
                ...
                try:
                    ...
                    # Keep the ModelRegistry's routability/health in lockstep
                    # with the separately persisted runtime lifecycle. This is
                    # the bridge that makes LIVE mean actually routable.
                    if model_registry is not None:
                        apply_probe_result(model_registry, result.probe)
                    results.append(result.to_dict())
                except Exception as exc:
                    runtime.mark_unavailable("runtime probe failed: %s" % type(exc).__name__)
                    results.append({"provider": runtime.provider, "model_id": runtime.model_id, "state": runtime.state, "changed": True, "error": type(exc).__name__})
            self._save()
            return {"time": checked_at, "results": results, **self.snapshot()}

    def snapshot(self) -> Dict[str, Any]:
        counts = self.registry.counts()
        return {"schema_version": 1, "running": self.running, "last_tick": self._last_tick, "interval_seconds": self.interval_seconds, "verification_ttl_seconds": self.monitor.verification_ttl_seconds, "counts": counts, "live": counts.get(RuntimeState.LIVE.value, 0), "runtimes": self.registry.snapshot()}

    def _probe(self, provider_name: str, model_id: str) -> RuntimeProbeResult:
        providers = getattr(self.fabric, "providers", None)
        if providers is None:
            return RuntimeProbeResult(model_id=model_id, ok=False, status="unavailable", reason="provider registry unavailable")
        provider = providers.get(provider_name)
        if provider is None:
            return RuntimeProbeResult(model_id=model_id, ok=False, status="unavailable", reason="provider is not registered")
        ...
        if callable(list_models):
            ...
            if model_id not in names:
                return RuntimeProbeResult(model_id=model_id, ok=False, latency_ms=latency, status="not_found", reason="exact model is not available")
    ...
    def _save(self) -> None:
        ...
        payload = self.snapshot(); temp = self.state_path.with_suffix(self.state_path.suffix + ".tmp")
        temp.write_text(json.dumps(payload, sort_keys=True), encoding="utf-8"); os.replace(str(temp), str(self.state_path))
