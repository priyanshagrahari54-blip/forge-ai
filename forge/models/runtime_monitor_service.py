            ...
            for runtime in self.registry.items():
                ...
                try:
                    ...
                    runtime.mark_unavailable(
                        "runtime probe failed: %s" % type(exc).__name__)
                    results.append({
    ...
    def _probe(self, provider_name: str, model_id: str) -> RuntimeProbeResult:
        ...
        if providers is None:
            return RuntimeProbeResult(
                model_id=model_id, ok=False, status="unavailable",
                reason="provider registry unavailable")
        ...
        return RuntimeProbeResult(
            model_id=model_id, ok=False, status="unverifiable",
            reason="provider has no exact model-list probe")
