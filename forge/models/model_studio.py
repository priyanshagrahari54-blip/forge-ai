    ...
    def validate_dataset(self, records: Iterable[Mapping[str, Any]]) -> Tuple[List[Dict[str, str]], DatasetReport]:
        ...
        for index, raw in enumerate(records):
            ...
            if not instruction or not output:
                rejected += 1
                issues.append(DatasetIssue(index, "EMPTY", "instruction and output are required", True))
                continue
            canonical = _canonical_json({"instruction": instruction, "output": output, "context": context})
            if len(canonical) > MAX_RECORD_CHARS:
                rejected += 1
                issues.append(DatasetIssue(index, "RECORD_BOUND", "record exceeds safety bound", True))
                continue
            if _secret_like(canonical):
                rejected += 1
                secret_count += 1
                issues.append(DatasetIssue(index, "SECRET", "secret-bearing sample rejected", True))
                continue
