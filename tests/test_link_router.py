...
def test_mode_server_refused_by_policy():
    ...
    assert decision.execution == "SERVER"  # decision labels the intent...
    assert "refused" in decision.reason    # ...but the reason records it
...
def test_mode_local_refuses_model_task_without_escalation_when_no_server():
    decision = decide("local", HEAVY, LocalPolicy(), SERVER_DOWN,
                      GOOD_RESOURCES)
    assert decision.execution == "LOCAL"   # refused: nothing else allowed
    assert "task requires a model" in decision.reason


def test_mode_local_delegates_to_server_when_available():
...
def test_hybrid_policy_disallow_server_stays_local():
    policy = LocalPolicy(allow_server=False)
