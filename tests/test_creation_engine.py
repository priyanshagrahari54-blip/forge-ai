...
from forge.agents.creation import (AgentCreationEngine, AgentPackage,
                                   TRANSITIONS, agent_identity, bump_version,
                                   spec_fingerprint)
...
def test_spec_rejects_unknown_and_repeated_tools():
    payload = full_spec()
    payload["tools"] = ["read_file", "launch-missiles"]
    with pytest.raises(ValueError):
        AgentSpec.from_dict(payload).check()
    payload = full_spec()
    payload["tools"] = ["memory_read", "memory_read"]
    with pytest.raises(ValueError):
        AgentSpec.from_dict(payload).check()
    # Aspirational tools with no runtime behind them are rejected too.
    payload = full_spec()
    payload["tools"] = ["read_file", "filesystem_read"]
    with pytest.raises(ValueError):
        AgentSpec.from_dict(payload).check()
...
def test_spec_rejects_bad_permissions():
    ...
    bad_permissions = [
        ...
        [{"resource": "filesystem", "operation": "read", "scope": "**",
          "risk": "LOW", "reason": ""}],
        # Operations must exist in the real A33 engine vocabulary.
        [{"resource": "git", "operation": "stage", "scope": "src/**",
          "risk": "LOW", "reason": "Stage files."}],
        [{"resource": "browser", "operation": "search", "scope": "web",
          "risk": "LOW", "reason": "Search the web."}],
        [{"resource": "network", "operation": "fetch", "scope": "docs/**",
          "risk": "LOW", "reason": "Fetch docs."}],
        # A blank terminal scope would be an unbounded shell grant.
        [{"resource": "terminal", "operation": "execute", "scope": "",
          "risk": "MEDIUM", "reason": "Run things."}],
    ]
    for permissions in bad_permissions:
        ...
        with pytest.raises(ValueError):
            AgentSpec.from_dict(payload).check()
    # Empty scopes are fine elsewhere (resource-default minimal scope).
    payload = dict(base)
    payload["permissions"] = [
        {"resource": "memory", "operation": "read", "scope": "",
         "risk": "LOW", "reason": "Recall own notes."}]
    AgentSpec.from_dict(payload).check()
...
def test_templates_are_least_privilege():
    ...
    assert "write_file" not in research.tools
    ...
    assert "run_tests" in coding.tools
...
def test_templates_differ_sensibly():
    ...
    assert "write_file" not in security.tools
    docs = build_template("documentation", "doc-writer")
    writes = [entry for entry in docs.permissions
              if entry.operation == "write"]
    assert writes and writes[0].scope == "docs/**"
    assert writes[0].resource == "filesystem"
...
def test_create_refuses_self_admin():
    engine = AgentCreationEngine()
    for actor in ("ego", "agent:ego", "forge-managed:ego", "agent:other"):
        with pytest.raises(ValueError, match="[Cc]annot create"):
            engine.create_from_spec(full_spec("ego"), created_by=actor)
    package = engine.create_from_spec(full_spec("ego"), created_by="op")
    assert package.created_by == "op"


def test_engine_creates_from_every_template():
    engine = AgentCreationEngine()
    for name in template_names():
        agent = "agent-%s" % name.replace("-", "")
        package = engine.create_from_template(name, agent,
                                              created_by="tester")
        assert package.state == "created"
        assert package.spec["template"] == name
...
def test_lifecycle_states_and_transitions():
    ...
    assert "validated" in TRANSITIONS["created"]
    assert "tested" in TRANSITIONS["validated"]
...
def test_lifecycle_pause_disable_rules():
    ...
    with pytest.raises(ValueError):
        engine.pause("pausy", actor="op")  # tested cannot pause
    with pytest.raises(ValueError):
        engine.resume("pausy", actor="op")  # only paused agents resume
    engine.enable("pausy", actor="op")
    engine.pause("pausy", actor="op")
    engine.disable("pausy", actor="op")  # paused -> disabled is legal
    assert engine.get("pausy").state == "disabled"
...
def test_enable_requires_passing_benchmark():
    ...
    with pytest.raises(ValueError):
        engine.enable("strict-one", actor="op")
...
def test_lifecycle_refuses_self_admin():
    engine = AgentCreationEngine()
    engine.create_from_template("coding", "ego-two", created_by="op")
    identity = agent_identity("ego-two")
    with pytest.raises(ValueError, match="cannot validate"):
        engine.validate("ego-two", actor=identity)
    with pytest.raises(ValueError, match="cannot benchmark"):
        engine.benchmark("ego-two", actor="ego-two")
    with pytest.raises(ValueError, match="cannot transition"):
        engine.pause("ego-two", actor="ego-two")
    with pytest.raises(ValueError, match="cannot update"):
        engine.update("ego-two", engine.get("ego-two").spec,
                      actor=identity)
    with pytest.raises(ValueError, match="cannot re-version"):
        engine.publish_version("ego-two", actor=identity)
    assert engine.get("ego-two").state == "created"


# -- spec updates ---------------------------------------------------------


def test_update_resets_lifecycle_and_bumps():
    engine = AgentCreationEngine()
    engine.create_from_template("coding", "owned", created_by="op")
    engine.validate("owned", actor="op")
    engine.benchmark("owned", actor="op")
    engine.enable("owned", actor="op")
    spec = dict(engine.get("owned").spec)
    spec["purpose"] = "A refined purpose for the same coder."
    package = engine.update("owned", spec, actor="op",
                            reason="purpose tweak")
    ...
    assert package.versions[-1]["spec_hash"] == spec_fingerprint(
        package.spec)


def test_update_rejects_retired_and_rename():
    engine = validated_engine()
    engine.retire("scout-one", actor="op")
    with pytest.raises(ValueError):
        engine.update("scout-one", full_spec("scout-one"), actor="op")
    engine = validated_engine()
    renamed = full_spec("someone-else")
    with pytest.raises(ValueError, match="cannot change"):
        engine.update("scout-one", renamed, actor="op")
...
def test_version_bumps_and_history():
    ...
    # A pure release marker keeps lifecycle state (the spec is unchanged).
    assert engine.get("scout-one").state == "validated"
    engine.retire("scout-one", actor="op")
    with pytest.raises(ValueError):
...
def test_grant_resets_lifecycle_and_bumps():
    engine = AgentCreationEngine()
    engine.create_from_template("coding", "trusted", created_by="op")
    engine.validate("trusted", actor="op")
    engine.benchmark("trusted", actor="op")
    engine.enable("trusted", actor="op")
    engine.grant_permission("trusted", 0, approver="op")
    package = engine.get("trusted")
    assert package.version == "1.0.1"
    assert package.state == "created"  # broader power re-earns trust
    engine.revoke_permission("trusted", 0, approver="op")
    package = engine.get("trusted")
...
def test_grants_reject_retired():
    engine = validated_engine()
    engine.retire("scout-one", actor="op")
    with pytest.raises(ValueError):
...
def test_export_import_arrives_ungranted():
    engine = validated_engine()
    engine.grant_permission("scout-one", 0, approver="operator")
...
def test_stores_are_isolated(tmp_path: Path):
    left = AgentCreationEngine(store_path=str(tmp_path / "left.json"))
    right = AgentCreationEngine(store_path=str(tmp_path / "right.json"))
    left.create_from_template("coding", "left-only", created_by="t")
    assert [package.name for package in left.list()] == ["left-only"]
    assert right.list() == []
    with pytest.raises(ValueError):
        right.get("left-only")
